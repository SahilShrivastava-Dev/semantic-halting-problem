...
development split, a \textbf{judge-free} semantic stopper reduces operational tokens
by \textbf{37\%} relative to \texttt{max\_iterations} at \textbf{parity quality},
whereas the full quality-gated variant is \textbf{counter-productive} because its
per-round judging dominates cost. An oracle that selects the best round attains
...
fall below $\varepsilon=0.06$ (Fig.~\ref{fig:dist}). Drafts therefore converge
quickly but with a heavy tail, so the $k\!=\!2$ patience window performs real work:
a single sub-threshold round does not trigger a halt.
...
(development split). The distance falls sharply after the first revision and then
hugs the halting threshold $\varepsilon$ (dashed), empirically supporting
Conjecture~\ref{conj:mono} while the heavy tail justifies the patience window.}
\label{fig:dist}
\end{figure}
...
\end{enumerate}
\noindent\textbf{Are these results promising?} The efficiency result is promising
and immediately actionable: a free, guaranteed-terminating stopper that removes
roughly a third of the token cost of the standard \texttt{max\_iterations} loop
without measurable quality loss. The quality result is not a failure but a
\emph{well-characterized open problem}: the oracle gap quantifies exactly how much
quality is left on the table and turns ``identify the best round'' into a concrete,
measurable target for future work.
