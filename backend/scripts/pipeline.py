...
def main() -> None:
    ...
    parser.add_argument("--provider", type=str, default=None, choices=["groq", "openai", "nvidia"])
    ...
    _run_phase(
        [py, "-m", "shp.agent_workflow", "--split", "train"] + extra,
    ...
    _run_phase(
        [py, "-m", "shp.agent_workflow", "--split", "val"] + extra,
        "PHASE 4: Validation (Live IS-Gain Tracking with learned weights)",
    )

    _run_phase(
        [py, os.path.join("tests", "test_information_score.py")] + eval_extra,
