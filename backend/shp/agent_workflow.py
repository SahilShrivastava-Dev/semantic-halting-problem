...
from shp.providers import get_llm, get_ragas_llm
from shp.halting import HaltConfig, shp_should_halt, derive_halt_reason, REASON_CONTINUE
...
def build_graph(
    ...
    Args:
        ...
        halt_config: Thresholds + per-signal ablation flags for the halt cascade.
                     Defaults to HaltConfig() (production thresholds, all signals on).
    ...
    halt_config = halt_config or HaltConfig()
    ...
    def check_convergence(state: WorkflowState) -> str:
        # Delegates the entire halt cascade to the single shared implementation in
        # halting.shp_should_halt — the live loop, the post-hoc reason derivation,
        # and the offline policy harness all share this one function, so they can
        # never drift (see halting.py / Lemma 2).
        loop_count   = state.get("loop_count", 0)
        history      = state.get("history", [])
        dist_history = state.get("distance_history", [])
        is_history   = state.get("is_score_history", [])
        last_feedback = history[-1].get("feedback", "") if history else ""
        ...
        if decision.halt:
            logger.info("🛑 [HALT — %s] round=%d", decision.reason.upper(), loop_count)
            _halt(decision.reason, loop_count, state, _emit)
            return "end"
...
def run_scenario(
    ...
    # Re-derive halt reason via the SAME shared function the live cascade used,
    # so the reported reason can never disagree with the reason that actually
    # stopped the loop (halting.derive_halt_reason / Lemma 2). This replaces the
    # old hand-duplicated if/elif ladder whose priority order silently differed
    # from check_convergence.
    last_feedback = history[-1].get("feedback", "") if history else ""
    halt_reason = derive_halt_reason(
        rounds=rounds,
        distance_history=dist_history,
        is_score_history=is_history,
        last_feedback=last_feedback,
