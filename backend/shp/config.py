...
PROVIDER_MODELS: dict[str, list[str]] = {
    "groq": [
        # mixtral-8x7b-32768 was removed: decommissioned by Groq (2024).
        # These two are the currently-serving free-tier models.
    ...
    # NVIDIA build (OpenAI-compatible, https://integrate.api.nvidia.com/v1).
    # Higher rate limits than Groq free tier — preferred for the experiments.
    "nvidia": [
...
# Judge defaults: use a STRONGER model than the agent where affordable, so the
# quality measurement is less noisy (addresses the "8B judge" validity risk).
...
# ─────────────────────────────────────────────────────────────
# Per-signal ablation flags
# ─────────────────────────────────────────────────────────────
# Toggle individual halt signals on/off for ablation experiments. Defaults keep
# every signal enabled (production behaviour). The MAX_ROUNDS failsafe is NOT
# toggleable here — it is the deterministic-termination guarantee (Theorem 1).
# Overridable via env for quick experiments; the harness sets them per-run.
def _env_bool(name: str, default: bool) -> bool:
    return os.getenv(name, str(default)).strip().lower() in ("1", "true", "yes", "on")

ENABLE_CRITIC_HALT: bool = _env_bool("ENABLE_CRITIC_HALT", True)
ENABLE_ENTROPY_HALT: bool = _env_bool("ENABLE_ENTROPY_HALT", True)
ENABLE_IS_GAIN_HALT: bool = _env_bool("ENABLE_IS_GAIN_HALT", True)
...
# ─────────────────────────────────────────────────────────────
# File paths (relative to backend/ — run commands from the backend/ directory)
# ─────────────────────────────────────────────────────────────
SCENARIOS_FILE: str = os.path.join("data", "test_scenarios.json")
