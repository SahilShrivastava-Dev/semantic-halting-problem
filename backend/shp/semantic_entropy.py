...
Provides SemanticEntropyCalculator — the geometric signal underlying the
Semantic Halting Problem (SHP) solution.

Core idea (semantic early-stopping):
    Every draft is mapped to a high-dimensional vector. We measure cosine
    distance between consecutive draft vectors. When that distance stays below
    CONVERGENCE_THRESHOLD for CONVERGENCE_PATIENCE consecutive rounds, the draft
    has stopped changing in meaning across iterations, so the loop is halted.
    This is a *patience-based early-stopping rule* on a semantic signal — the
    same family as early stopping in ML training — NOT a Banach contraction.

    We deliberately do NOT claim the Writer→Critic update is a contraction
    mapping: LLM generation has no proven Lipschitz constant < 1 and is not
    deterministic across API calls, so a Banach fixed-point guarantee would be
    unsupported. What IS guaranteed is deterministic termination via the
    MAX_ROUNDS failsafe (see halting.py, Theorem 1). The monotone/non-increasing
    behaviour of the distance sequence is treated as an empirical conjecture and
    measured, not assumed (see theory_checks.empirical_monotonicity_report).
...
Metric choice rationale:
    ...
    cosine similarity — making this both well-motivated and computationally
